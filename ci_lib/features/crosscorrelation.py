...
class CrossCorrelations(Features):
    ...
    @staticmethod
    def create(data, means=None, covs=None, acovs=None, timelag=0, label = None, logger=LOGGER):
        if covs is None:
            covs = Covariances.create(data, means, True, logger)._feature
        elif isinstance(covs, Covariances):
            covs = np.copy(acovs._feature)
        if acovs is None:
            acovs = AutoCovariances.create(data, means, covs, timelag, None, True, logger)._feature
        elif isinstance(acovs, CrossCovariances):
