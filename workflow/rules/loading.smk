...
def sessions_input(wildcards):
    ...
    input = {
        ...
        "Vc": [  f"resources/experiment/{subject_id}/{date}/SVD_data/Vc.mat" for subject_id,dates in subject_dates_dict.items() for date in dates ], #TODO only works for new vc files, currently done this way to make sure only news vcs are used
