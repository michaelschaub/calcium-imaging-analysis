...
#from loading import load_task_data_as_pandas_df #extract_session_data_and_save, fix_dates
import load_task_data_as_pandas_df
...
def extract_all_trials(cond_list, sf, root_paths, mouse_ids, run_extraction, save=False):
    ...
    if run_extraction:
        sessions = load_task_data_as_pandas_df.extract_session_data_and_save(root_paths=root_paths, mouse_ids=mouse_ids, reextract=True)
    else:
        # load data
        sessions = pkl.load(open(root_paths[0] / "task_data/MS_task_V2_1_extracted_data.pkl", 'rb'))
        sessions = load_task_data_as_pandas_df.fix_dates(sessions)
